...
## ⚙️ 1. Como Executar o Backend (`/backend`)
...
2. Crie ou ajuste o arquivo `.env` na pasta `mobile` apontando a API para o seu **IP local na rede Wi-Fi**:
   ...
   ```
   *(Substitua `192.168.1.X` pelo IP do seu computador na rede local).*
...
1. **🎨 Design System Identico entre Web & Mobile:**
   * Cabeçalho de navegação (Navbar) 1:1 com badge de usuário ativo.
   * Cards de Métricas/Dashboard (Total, Ativos e Inativos).
   * Faixa indicadora visual de cor na borda esquerda de cada produto (`● ATIVO` / `● INATIVO`).
2. **⚡ Busca com Debounce (Mobile):** Delay otimizado de 400ms no campo de pesquisa mobile para evitar sobrecarga de requisições.
3. **🖼️ Preview de Foto & Upload:** Suporte a seleção e visualização prévia da foto do produto antes de salvar.
4. **🔒 Autenticação JWT & Criptografia:** Criptografia Bcrypt para senhas e tokens JWT armazenados com segurança.
5. **🐘 Banco PostgreSQL em Nuvem:** Preparado para conexão em nuvem via Neon DB com migrações automáticas.
